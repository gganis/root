...
Int_t TH3::SetRangesFromList(TList *axl)
{
   ...
   if (nax && nay && naz) {
      if (nax->GetNbins() > 0 && nax->GetXmax() > nax->GetXmin() &&
          nay->GetNbins() > 0 && nay->GetXmax() > nay->GetXmin() &&
          naz->GetNbins() > 0 && naz->GetXmax() > naz->GetXmin()) {
         if (gDebug > 1)
            Info("SetRangesFromList", "found|had for '%s' :\n"
                                ...
                                axl->GetName(),
                                nax->GetNbins(), fXaxis.GetNbins(), nax->GetXmin(),
                                fXaxis.GetXmin(), nax->GetXmax(), fXaxis.GetXmax(),
                                nay->GetNbins(), fYaxis.GetNbins(), nay->GetXmin(),
                                fYaxis.GetXmin(), nay->GetXmax(), fYaxis.GetXmax(),
                                naz->GetNbins(), fZaxis.GetNbins(), naz->GetXmin(),
                                fZaxis.GetXmin(), naz->GetXmax(), fZaxis.GetXmax());
         SetBins(nax->GetNbins(), nax->GetXmin(), nax->GetXmax(),
                 nay->GetNbins(), nay->GetXmin(), nay->GetXmax(),
...
{
    return (fXaxis.GetXmax() <= fXaxis.GetXmin() ||
            fYaxis.GetXmax() <= fYaxis.GetXmin() ||
            fZaxis.GetXmax() <= fZaxis.GetXmin()) ? kTRUE : kFALSE;
}
