...
Int_t TH1::SetRangesFromList(TList *axl)
{
   ...
   if (nax) {
      if (nax->GetNbins() > 0 && nax->GetXmax() > nax->GetXmin()) {
         if (gDebug > 1)
            Info("SetRangesFromList", "found|had for '%s' : bins, xmin, xmax : %d|%d, %f|%f, %f|%f",
                                axl->GetName(), nax->GetNbins(), fXaxis.GetNbins(), nax->GetXmin(),
                                fXaxis.GetXmin(), nax->GetXmax(), fXaxis.GetXmax());
         SetBins(nax->GetNbins(), nax->GetXmin(), nax->GetXmax());
         rc = 0;
      } else {
         Warning("SetRangesFromList", "found for '%s' inconsistent bins, xmin, xmax (%d, %f, %f)",
                                axl->GetName(), nax->GetNbins(), nax->GetXmin(), nax->GetXmax());
      }
   } else {
...
////////////////////////////////////////////////////////////////////////////////
/// Get identifier for list wth ranges
///
...
TString TH1::GetNameForRanges()
{
   ...
   while (dir && strcmp(dir->GetName(), "Rint") && !strstr(dir->GetName(), "__TThreaded_dir_")) {
      onm.Insert(0, TString::Format("%s/", dir->GetName()));
   ...
   if (CanExtendAllAxes() || HasNoLimits()) {
      ...
      if (funcb) {
         ...
         if (raxl) {
            ...
            if (needaxes) {
               Warning("BufferEmpty", "callback: received inconsistent information (%s, %d)",
                                      raxl->GetName(), raxl->GetSize());
            }
         }
         // Cleanup
         if (axl) delete axl;
      ...
      if (needaxes) {
         ...
         {  fgRefSyncMtx.ReadLock();
            if (!fgRefSync) fgRefSync = new THashList;
            TList *raxl = (TList *) fgRefSync->FindObject(onm);
            if (raxl) {
               needaxes = (SetRangesFromList(raxl) != 0) ? kTRUE : kFALSE;
               if (needaxes) {
                  Warning("BufferEmpty", "refsync:1 received inconsistent information (%s, %d)",
                                         raxl->GetName(), raxl->GetSize());
               }
            }
      ...
      if (needaxes) {
         ...
         if (raxl) {
            needaxes = (SetRangesFromList(raxl) != 0) ? kTRUE : kFALSE;
            if (needaxes) {
               Warning("BufferEmpty", "refsync:2 received inconsistent information (%s, %d)",
                                         raxl->GetName(), raxl->GetSize());
            }
         }
