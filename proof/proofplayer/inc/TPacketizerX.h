...
#ifndef ROOT_TPacketizerX
#define ROOT_TPacketizerX
...
class TPacketizerX : public TVirtualPacketizer {
...
private:
   ...
   TPacketizerX();
   TPacketizerX(const TPacketizer&);     // no implementation, will generate
   void operator=(const TPacketizerX&);  // error on accidental usage
...
public:
   TPacketizerX(TDSet *dset, TList *slaves, Long64_t first, Long64_t num,
                TList *input, TProofProgressStatus *st);
   virtual ~TPacketizerX();
   ...
   ClassDef(TPacketizerX,0)  //Generate work packets for parallel processing
};
