...
class TPacketizerX::TFileStat : public TObject {
...
TPacketizerX::TFileStat::TFileStat(TFileNode *node, TDSetElement *elem)
...
class TPacketizerX::TFileNode : public TObject {
...
public:
   ...
   Int_t Compare(const TObject *other) const
   {
      ...
      if (!obj) {
         Error("Compare", "input is not a TPacketizerX::TFileNode object");
         return 0;
      }
...
TPacketizerX::TFileNode::TFileNode(const char *name)
...
class TPacketizerX::TSlaveStat : public TVirtualPacketizer::TVirtualSlaveStat {

friend class TPacketizerX;
...
TPacketizerX::TSlaveStat::TSlaveStat(TSlave *slave)
...
TPacketizerX::TSlaveStat::~TSlaveStat()
{
   // Cleanup
...
TProofProgressStatus *TPacketizerX::TSlaveStat::AddProcessed(TProofProgressStatus *st)
{
...
ClassImp(TPacketizerX)

//______________________________________________________________________________
TPacketizerX::TPacketizer(TDSet *dset, TList *slaves, Long64_t first,
                         Long64_t num, TList *input, TProofProgressStatus *st)
            : TVirtualPacketizer(input, st)
{
   // Constructor

   PDB(kPacketizer,1) Info("TPacketizerX", "Enter (first %lld, num %lld)", first, num);
   ...
   if (!fProgressStatus) {
      Error("TPacketizerXAdaptive", "No progress status");
      return;
   }
   ...
   if (TProof::GetParameter(input, "PROOF_MaxSlavesPerNode", maxSlaveCnt) == 0) {
      if (maxSlaveCnt < 1) {
         Warning("TPacketizerX", "PROOF_MaxSlavesPerNode must be grater than 0");
         maxSlaveCnt = 0;
      }
   } else {
   ...
   if (maxSlaveCnt > 0) {
      ...
      PDB(kPacketizer,1)
         Info("TPacketizerX", "setting max number of workers per node to %ld", fMaxSlaveCnt);
   }
   ...
   PDB(kPacketizer,2) Info("TPacketizerX", "processing range: first %lld, num %lld", first, num);
   ...
   while (( e = (TDSetElement*)dset->Next())) {
      ...
      PDB(kPacketizer,2)
         Info("TPacketizerX", " --> '%s'", e->GetFileName());
      PDB(kPacketizer,2)
         Info("TPacketizerX", " --> first %lld, num %lld (cur %lld)", eFirst, eNum, cur);
      ...
      if (!e->GetEntryList()){
         ...
         if (cur + eNum < first) {
            cur += eNum;
            PDB(kPacketizer,2)
               Info("TPacketizerX", " --> skip element cur %lld", cur);
            continue;
         }
         ...
         if (num != -1 && (first+num <= cur)) {
            cur += eNum;
            PDB(kPacketizer,2)
               Info("TPacketizerX", " --> drop element cur %lld", cur);
            continue; // break ??
         }
         ...
         if (cur <= first || (num != -1 && (first+num <= cur+eNum))) {
            ...
            if (cur <= first) {
               ...
               PDB(kPacketizer,2)
                  Info("TPacketizerX", " --> adjust start %lld and end %lld",
            ...
            if (num != -1 && (first+num <= cur+eNum)) {
               ...
               PDB(kPacketizer,2)
                  Info("TPacketizerX", " --> adjust end %lld", first + num - cur);
               inRange = kTRUE;
            }
            ...
            PDB(kPacketizer,2)
               Info("TPacketizerX", " --> increment 'cur' by %lld", eNum);
            cur += eNum;
         }
      ...
      PDB(kPacketizer,2)
         Info("TPacketizerX", " --> next cur %lld", cur);
   ...
   PDB(kPacketizer,1)
      Info("TPacketizerX", "processing %lld entries in %d files on %d hosts",
   ...
   if (fFileNodes->GetSize() == 0) {
      Info("TPacketizerX", "no valid or non-empty file found: setting invalid");
   ...
   PDB(kPacketizer,1) Info("TPacketizerX", "Base Packetsize = %lld", fPacketSize);

   if (!fValid)
      SafeDelete(fProgress);

   PDB(kPacketizer,1) Info("TPacketizerX", "Return");
}

//______________________________________________________________________________
TPacketizerX::~TPacketizer()
{
   // Destructor.
...
//______________________________________________________________________________
TPacketizerX::TFileStat *TPacketizer::GetNextUnAlloc(TFileNode *node)
{
...
//______________________________________________________________________________
TPacketizerX::TFileNode *TPacketizer::NextUnAllocNode()
{
   // Get next unallocated node.

   fUnAllocated->Sort();
   PDB(kPacketizer,2) {
      cout << "TPacketizerX::NextUnAllocNode()" << endl;
      fUnAllocated->Print();
   }
...
//______________________________________________________________________________
void TPacketizerX::RemoveUnAllocNode(TFileNode * node)
{
...
//______________________________________________________________________________
TPacketizerX::TFileStat *TPacketizer::GetNextActive()
{
...
//______________________________________________________________________________
TPacketizerX::TFileNode *TPacketizer::NextActiveNode()
{
   // Get next active node.

   fActive->Sort();
   PDB(kPacketizer,2) {
      cout << "TPacketizerX::NextActiveNode()" << endl;
      fActive->Print();
   }
...
//______________________________________________________________________________
void TPacketizerX::RemoveActive(TFileStat *file)
{
...
//______________________________________________________________________________
void TPacketizerX::RemoveActiveNode(TFileNode *node)
{
...
//______________________________________________________________________________
void TPacketizerX::ValidateFiles(TDSet *dset, TList *slaves, Long64_t maxent, Bool_t byfile)
{
...
//______________________________________________________________________________
Long64_t TPacketizerX::GetEntriesProcessed(TSlave *slave) const
{
...
//______________________________________________________________________________
Float_t TPacketizerX::GetCurrentRate(Bool_t &all)
{
...
//______________________________________________________________________________
TDSetElement *TPacketizerX::GetNextPacket(TSlave *sl, TMessage *r)
{
   // Get next packet
...
//______________________________________________________________________________
Int_t TPacketizerX::GetActiveWorkers()
{
